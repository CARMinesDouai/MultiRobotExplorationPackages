...
public:
    OGMap(int width= 1, int height= 1, const Float2 & origine= 0.f, float resolution= 1.f);
    ~OGMap();
    ...
    {
        return (vg + Float2(0.5f)) * a_resolution + a_origin;
    }
    virtual Float2 worldToGrid(const Float2 vw)const
    {
        return (vw - a_origin) / a_resolution - Float2(0.5f);
    }
